...
4. [Santander Product Recommendation](https://www.kaggle.com/c/santander-product-recommendation) Built a recommendation system to cross sell products to existing customers (multi-class classification). Explained the approach in a blog. Used Matplotlib and Seaborn to visualize products distribution by various features. Converted the dataset to be used as a multi-class classification problem. Created new feature lags and statistics metrics such as min, max and standard deviations. Modeled using XGBoost and Matrix Factorization techniques such as SVD. &nbsp;&nbsp;&nbsp;&nbsp;[#RecommenderSystem] [#MultiClassClassification]
5. [Toxic Comment Classification](https://www.kaggle.com/competitions/jigsaw-multilingual-toxic-comment-classification/overview) Built a multi-headed model that predicts probabilities of different types of toxicity such as threats, obscenity, insults, and identity-based in the comments. Used Matplotlib and Seaborn to visualize distribution of target classes, correlation of categorical variables using confusion matrix and frequently used words using WordCloud. Created new features and visualized those features to understand whether longer comments are more toxic, etc. Vectorized the words using TF-IDF and used LogisticRegression with and without additional features. Corrected spelling mistakes using FastText embeddings and trained a RNN network with LSTM and GRU cells using Keras.&nbsp;&nbsp;&nbsp;&nbsp;[#NLP] [#Regression]
...
10. [StackOverFlow Tag Prediction](https://github.com/rthothad/Kaggle/tree/master/StackOverTagPredictor) - Built multi-class classification models to suggest tags based on the title and question text. Used Matplotlib and Seaborn to visualize distribution of tags, tags per question and most frequent tags. Preprocessed the data using NLTK to remove stopwords, special characters, HTML tags and SnowballStemmer to stem the words. Saved the cleaned data to SQLite DB. Vectorized the words using TF-IDF and used LogisticRegression with One Vs Rest Classifier and SGDClassifier with hinge loss. Used GridSearch to tune hyperparameters.
11. Click Prediction - [Amex 2018] (https://github.com/rthothad/AV/tree/master/Amex2018) - Built a model to predict the probability of the next page a user will visit.  Created new features such as counts, number of times a user visited a page, how long since his last visit, mean encoded features against the target variable, etc. Tuned hyperparameters using RandomSearch and selected features using forward feature selection method. Trained a CatBoost and LightGBM model. &nbsp;&nbsp;&nbsp;&nbsp;#TimeSeries
12. Demand Forecasting - [Food Demand Forecasting](https://github.com/rthothad/AV/tree/master/Genpact Food Demand Forecasting) – Built a model to predict 10 weeks out demand for meals. Built many interaction features and statistical features such as sum, count, min, max, mean and mode. Dropped non-correlated features and trained a RandomForestRegressor and XGBoost model.  &nbsp;&nbsp;&nbsp;&nbsp;#TimeSeries
## Published Articles
